...
- **Network Simulation**: Redirect dead/malicious URLs to INetSim for safe analysis (NEW!)

## 🌐 Network Simulation Integration

Pack-A-Mal now supports **INetSim integration** for analyzing packages with network-based malicious behavior:
...
./scripts/setup_network_simulation.sh  # Linux/Mac
# or
.\scripts\setup_network_simulation.ps1  # Windows

# Enable network simulation
export OSSF_NETWORK_SIMULATION_ENABLED=true
export OSSF_INETSIM_DNS_ADDR=172.20.0.2:53

# Analyze a package with network behavior
./scripts/run_analysis.sh --ecosystem pypi --package sample_packages/malicious_network_package
```

📖 **Full Documentation**: See [NETWORK_SIMULATION_GUIDE.md](NETWORK_SIMULATION_GUIDE.md) for complete setup and usage instructions.
